...
function normalizeStatus(value) { return String(value ?? "").split('"').join("").split("'").join("").trim().toUpperCase(); }
function toNumber(value) { const num = Number(value); return Number.isFinite(num) ? num : 0; }
function getOrderDate(order) { return order?.fechaPedido || order?.createdAt || null; }
function getOrderTotal(order) { return toNumber(order?.montoTotal ?? order?.totalAmount ?? 0); }
function getOrderItems(order) { if (Array.isArray(order?.detalles)) return order.detalles; if (Array.isArray(order?.items)) return order.items; return []; }
function getLineTotal(detail, quantity) {
    const subtotal = Number(detail?.subTotal); if (Number.isFinite(subtotal)) return subtotal;
    const precioEnCompra = Number(detail?.precioEnCompra); if (Number.isFinite(precioEnCompra)) return precioEnCompra * quantity;
    const unitPrice = Number(detail?.unitPrice); if (Number.isFinite(unitPrice)) return unitPrice * quantity;
    return 0;
}
...
export default function DashboardPage() {
    ...
    const formatearPrecio = (precio) => new Intl.NumberFormat("es-CL", { style: "currency", currency: "CLP", maximumFractionDigits: 0 }).format(toNumber(precio));

    async function fetchPedidoPorId(id) {
        if (!String(id).trim()) return;
        try {
            setCargandoPedido(true); setErrorPedido(null); setPedidoDetalle(null);
            const data = await obtenerPedidoPorId(id);
            setPedidoDetalle(data);
        } catch (e) {
            setErrorPedido(e?.status === 404 ? "Pedido no encontrado." : "Error al buscar el pedido.");
    ...
    useEffect(() => {
        ...
        async function cargarDatos() {
            ...
            try {
                const [prods, usrs, peds] = await Promise.all([obtenerTodosLosProductosAdmin(), obtenerUsuarios(), obtenerTodosLosPedidos()]);
                if (!activo) return;
                setProductos(Array.isArray(prods) ? prods : []);
                setUsuarios(Array.isArray(usrs) ? usrs : []);
                setPedidos(Array.isArray(peds) ? peds : []);
            } catch (e) {
                if (!activo) return; setError(e?.message || "No se pudieron cargar los datos del panel.");
    ...
    const { totalProductos, totalUsuarios, productosBajoStock, bajoStockCount, pctOk, pctLow, totalPedidos, pedidosPendientes, ingresosTotales, ultimosPedidos } = useMemo(() => {
        ...
        const pedidosPendientes = pedidos.filter((p) => { const status = normalizeStatus(p?.estado ?? p?.status); return status === "PENDIENTE" || status === "PENDING"; }).length;
        const ingresosTotales = pedidos.filter((p) => { const status = normalizeStatus(p?.estado ?? p?.status); return ["COMPLETADO", "COMPLETED", "DELIVERED", "PAID", "SHIPPED"].includes(status); }).reduce((sum, p) => sum + getOrderTotal(p), 0);
        const ultimosPedidos = [...pedidos].sort((a, b) => toNumber(b?.id) - toNumber(a?.id)).slice(0, 5);
        return { totalProductos, totalUsuarios, productosBajoStock, bajoStockCount, pctOk, pctLow, totalPedidos, pedidosPendientes, ingresosTotales, ultimosPedidos };
            ...
            <div className="container px-3 px-md-5 py-4">
                ...
                <motion.div variants={containerVariants} initial="hidden" animate="visible">
                    <div className="row g-4 mb-4">
                        <motion.div className="col-12 col-md-4" variants={cardVariants}>
                            <motion.div whileHover={{ y: -5 }} className="admin-card h-100 d-flex flex-column justify-content-between text-start">
                                ...
                                <div className="mt-4"><Link to="/admin/inventario" className="admin-btn-outline w-100 text-center">Gestionar Inventario</Link></div>
                            </motion.div>
                        </motion.div>
                        ...
                        <motion.div className="col-12 col-md-4" variants={cardVariants}>
                            <motion.div whileHover={{ y: -5 }} className="admin-card h-100 d-flex flex-column justify-content-between text-start">
                                ...
                                <div className="mt-4"><Link to="/admin/usuarios" className="admin-btn-outline w-100 text-center">Ver Comunidad</Link></div>
                            </motion.div>
                        </motion.div>
                        ...
                        <motion.div className="col-12 col-md-4" variants={cardVariants}>
                            <motion.div whileHover={{ y: -5 }} className={`admin-card h-100 d-flex flex-column justify-content-between text-start ${bajoStockCount > 0 ? 'critical-border' : ''}`}>
                                ...
                                <div className="mt-4"><Link to="/admin/inventario" className={`admin-btn-solid w-100 text-center ${bajoStockCount > 0 ? 'btn-danger' : ''}`}>Revisar Alertas</Link></div>
                            </motion.div>
                        </motion.div>
                    </div>
                            ...
                            <motion.div whileHover={{ scale: 1.02 }} className="admin-card p-4 h-100 feature-card-1 border-0 text-start">
                                <div className="d-flex justify-content-between align-items-start">
                                    ...
                                    <div className="d-flex flex-column align-items-end z-2">
                                        ...
                                        <Link to="/admin/pedidos" className="admin-btn-outline text-dark bg-white border-0">Despachar Órdenes</Link>
                                    </div>
                                </div>
                            </motion.div>
                        </motion.div>
                        ...
                        <motion.div className="col-12 col-md-6" variants={cardVariants}>
                            <motion.div whileHover={{ scale: 1.02 }} className="admin-card p-4 h-100 feature-card-2 border-0 text-start">
                                <div className="d-flex justify-content-between align-items-start">
                                    ...
                                    <div className="z-2"><div className="admin-icon-large"><i className="fas fa-chart-line"></i></div></div>
                                </div>
                            </motion.div>
                        </motion.div>
                    </div>
                    ...
                    <motion.div className="admin-card p-4 text-start" variants={cardVariants}>
                        ...
                        <p className="fw-medium mb-3" style={{ fontSize: "13px", color: "#86868b" }}>{productosBajoStock.length} productos requieren reabastecimiento (stock &lt; 5 unidades).</p>
                        <div className="admin-progress-bg">
                            <motion.div className="admin-progress-fill" initial={{ width: 0 }} animate={{ width: `${pctOk}%` }} transition={{ duration: 1.5, ease: "easeOut" }}></motion.div>
                        </div>
                    </motion.div>
                </motion.div>
            </div>
