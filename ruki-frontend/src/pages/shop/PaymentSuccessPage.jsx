...
export default function PaymentSuccessPage() {
    ...
    const containerVariants = { hidden: { opacity: 0, scale: 0.95 }, visible: { opacity: 1, scale: 1, transition: { duration: 0.5, ease: "easeOut", staggerChildren: 0.1 } } };
    const itemVariants = { hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0 } };
    const checkmarkVariants = { hidden: { scale: 0, opacity: 0 }, visible: { scale: 1, opacity: 1, transition: { type: "spring", stiffness: 200, damping: 15, delay: 0.2 } } };
    ...
    return (
        <main className="success-page-wrapper">
            ...
            <motion.div className="success-card" variants={containerVariants} initial="hidden" animate="visible">
                ...
                <motion.h2 className="fw-bolder text-white mb-3" variants={itemVariants}>¡Pago Exitoso!</motion.h2>
                ...
                <motion.div className="d-flex flex-column gap-3 px-3 mt-4" variants={itemVariants}>
                    <button className="success-btn-primary w-100" onClick={() => navigate('/productos')}>Seguir comprando</button>
                    <button className="success-btn-outline w-100" onClick={() => navigate('/')}>Volver al inicio</button>
                </motion.div>
            </motion.div>
        </main>
    );
