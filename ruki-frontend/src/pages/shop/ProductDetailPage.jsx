...
export default function ProductDetailPage() {
    ...
    const currentStock = producto?.variants?.length > 0 ? (selectedVariant ? selectedVariant.stock : 0) : producto?.stock;

    const containerVariants = { hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1, delayChildren: 0.2 } } };
    const itemVariants = { hidden: { opacity: 0, y: 15 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } } };
    const imageVariants = { hidden: { opacity: 0, scale: 0.95 }, visible: { opacity: 1, scale: 1, transition: { duration: 0.6, ease: "easeOut" } } };
    ...
    return (
        <main className="prod-detail-wrapper pb-5">
                ...
                {loading ? (
                    <div className="d-flex flex-column justify-content-center align-items-center" style={{ minHeight: '50vh' }}>
                        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="spin-animation mb-3" style={{color: '#86868b'}}><path d="M21 12a9 9 0 1 1-6.219-8.56"></path></svg>
                            <p className="fw-semibold" style={{color: '#a1a1a6'}}>Cargando detalles del producto...</p>
                        </motion.div>
                    </div>
                ) : error || !producto ? (
                        ...
                        <motion.div className="col-12 col-md-6 col-lg-5 offset-lg-1 d-flex flex-column pt-0 pt-md-3 px-3" variants={containerVariants} initial="hidden" animate="visible">
                            ...
                            <motion.div className="mt-4 pt-2" variants={itemVariants}>
                                ...
                                <div className="shipping-info-box">
                                    ...
                                    <div className="shipping-item"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path><polyline points="9 22 9 12 15 12 15 22"></polyline></svg><span>Retiro en tienda gratuito.</span></div>
                                    <div className="shipping-item"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg><span>Pago 100% seguro garantizado.</span></div>
                                </div>
                            </motion.div>
                        </motion.div>
                    </div>
            ...
            <div className="toast-fixed-container">
                <AnimatePresence>
                    {toast.mostrar && (
                        <motion.div className={`ios-toast-notification ${toast.tipo === 'error' ? 'error' : 'success'}`} initial={{ opacity: 0, y: 50, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.9 }} transition={{ type: "spring", stiffness: 300, damping: 25 }}>
                            {toast.tipo === 'error' ? <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ff3b30" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="me-3"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg> : <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#34c759" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="me-3"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>}
                            <div className="toast-body-text">{toast.mensaje}</div>
                        </motion.div>
                    )}
                </AnimatePresence>
            </div>
