...
export default function ProfilePage() {
    ...
    const [addressForm, setAddressForm] = useState({ street: '', city: '', region: '', zipCode: '', referenceInfo: '' });
    ...
    const cargarDatos = async () => {
        try {
            const [profileData, addressData] = await Promise.all([
                obtenerMiPerfil(),
                obtenerDireccionesActivasPorUsuario(usuario.id)
            ]);
    ...
    const handleUpdateProfile = async (e) => {
        ...
        try {
            await actualizarUsuario(perfil.id, { ...formData });
    ...
    const cardVariants = { hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6, ease: [0.2, 0.65, 0.3, 0.9] } } };
    const listItemVariants = { hidden: { opacity: 0, x: -10 }, visible: { opacity: 1, x: 0 } };
        ...
        <div className="profile-main-wrapper">
            ...
            <div className="container mt-5 mb-5 position-relative" style={{ zIndex: 1 }}>
                                    ...
                                    <form onSubmit={handleUpdateProfile}>
                                        <div className="ios-input-group mb-3">
                                            ...
                                            <div className="ios-input-wrapper">
                                                ...
                                                <input type="text" className="ios-input-glass w-100" disabled={!editMode} required value={formData.firstName} onChange={(e) => setFormData({ ...formData, firstName: e.target.value })} />
                                            </div>
                                        </div>
                                        <div className="ios-input-group mb-3">
                                            ...
                                            <div className="ios-input-wrapper">
                                                ...
                                                <input type="text" className="ios-input-glass w-100" disabled={!editMode} required value={formData.lastName} onChange={(e) => setFormData({ ...formData, lastName: e.target.value })} />
                                            </div>
                                        </div>
                                        <div className="ios-input-group mb-3">
                                            ...
                                            <div className="input-helper-text mt-1"><i className="fas fa-lock me-1"></i>No editable por seguridad</div>
                                        </div>
                                        ...
                                        <div className="ios-input-group mb-4 mt-4 pt-4 border-top border-dark" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
                                            ...
                                            <motion.button type="button" whileTap={{ scale: 0.95 }} className="ios-btn ios-btn-outline w-100 d-flex justify-content-center align-items-center" onClick={() => navigate('/forgot-password')}>
                                                <i className="fas fa-key me-2"></i> Cambiar mi contraseña
                                            </motion.button>
                                            <div className="input-helper-text mt-2 text-center opacity-75">Te enviaremos un enlace seguro a tu correo.</div>
                                        </div>
                        ...
                        <motion.div className="col-md-7" variants={cardVariants}>
                            <div className="profile-card-glass h-100 d-flex flex-column">
                                ...
                                <div className="p-4 border-top border-dark" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
                                    <motion.button whileTap={{ scale: 0.98 }} className="ios-btn ios-btn-outline w-100" onClick={() => setShowAddressModal(true)}>
                ...
                <Modal show={showAddressModal} onHide={() => setShowAddressModal(false)} centered backdrop="static" contentClassName="profile-modal-glass">
                    <div className="p-4">
                        <div className="d-flex justify-content-between align-items-center mb-4">
                            ...
                            <button type="button" className="btn-close-glass" onClick={() => setShowAddressModal(false)}><i className="fas fa-times"></i></button>
                        </div>
                        <form onSubmit={handleAddAddress}>
                            <div className="ios-input-group mb-3">
                                ...
                                <div className="ios-input-wrapper">
                                    ...
                                    <input type="text" className="ios-input-glass w-100" required placeholder="Ej: Av. Providencia 1234" value={addressForm.street} onChange={e => setAddressForm({ ...addressForm, street: e.target.value })} />
                                </div>
                            </div>
                            ...
                            <div className="row g-3 mb-3">
                                <div className="col-6 ios-input-group">
                                    ...
                                    <div className="ios-input-wrapper">
                                        ...
                                        <select className="ios-select-glass w-100" required value={addressForm.region} onChange={e => setAddressForm({ ...addressForm, region: e.target.value, city: '' })}>
                                ...
                                <div className="col-6 ios-input-group">
                                    ...
                                    <div className="ios-input-wrapper">
                                        ...
                                        <select className="ios-select-glass w-100" required disabled={!addressForm.region} value={addressForm.city} onChange={e => setAddressForm({ ...addressForm, city: e.target.value })}>
                                            <option value="" style={{ color: '#a1a1a6', background: '#111111' }}>Seleccionar...</option>
                                            {comunasDisponibles.map((c, i) => <option key={i} value={c} style={{ color: '#ffffff', background: '#111111' }}>{c}</option>)}
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="ios-input-group mb-3">
                                ...
                                <div className="ios-input-wrapper">
                                    ...
                                    <input type="text" className="ios-input-glass w-100" placeholder="Ej: 7500000" value={addressForm.zipCode} onChange={e => setAddressForm({ ...addressForm, zipCode: e.target.value })} />
                                </div>
                            </div>
                            <div className="ios-input-group mb-4">
                                <label className="ios-label">Referencias (Opcional)</label>
                                <textarea className="ios-input-glass w-100" rows="2" placeholder="Ej: Casa esquina, portón negro..." style={{ resize: 'none', paddingLeft: '16px' }} value={addressForm.referenceInfo} onChange={e => setAddressForm({ ...addressForm, referenceInfo: e.target.value })}></textarea>
                            </div>

                            <div className="d-flex gap-2">
                                <motion.button whileTap={{ scale: 0.95 }} type="button" className="ios-btn ios-btn-outline w-100" onClick={() => setShowAddressModal(false)}>CANCELAR</motion.button>
                                <motion.button whileTap={{ scale: 0.95 }} type="submit" className="ios-btn ios-btn-dark w-100" disabled={actionLoading}>
                ...
                <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered contentClassName="profile-modal-glass border-danger-glass">
                    <div className="p-4 text-center">
                        <div className="mb-4">
                            ...
                            <p className="text-secondary small mb-0" style={{ color: '#a1a1a6' }}>Esta acción no se puede deshacer. La dirección ya no estará disponible para futuros pedidos.</p>
                        </div>
                        <div className="d-flex gap-2">
                            <motion.button whileTap={{ scale: 0.95 }} type="button" className="ios-btn ios-btn-outline w-100" onClick={() => setShowDeleteModal(false)}>CANCELAR</motion.button>
                            <motion.button whileTap={{ scale: 0.95 }} type="button" className="ios-btn w-100 border-0" style={{ backgroundColor: '#ff3b30', color: '#ffffff' }} disabled={actionLoading} onClick={confirmDeleteAddress}>
                ...
                <div className="profile-toast-container">
                    <AnimatePresence>
                        {toast.mostrar && (
                            <motion.div className={`profile-ios-toast-glass ${toast.tipo === 'error' ? 'error' : 'success'}`} initial={{ opacity: 0, y: 50, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.9 }} transition={{ type: "spring", stiffness: 300, damping: 25 }}>
                                {toast.tipo === 'success' ? <i className="fas fa-check-circle text-success me-3 fs-5"></i> : <i className="fas fa-exclamation-triangle text-danger me-3 fs-5"></i>}
