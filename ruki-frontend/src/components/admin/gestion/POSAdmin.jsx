...
export function POSAdmin() {
    ...
    const agregarAlPOS = (producto, tallaElegida) => {
        setCarritoPOS(prev => {
            ...
            if (itemExistente) {
                return prev.map(item => (item.id === producto.id && item.size === tallaElegida) 
    ...
    const quitarDelPOS = (id, size) => {
        setCarritoPOS(prev => prev.filter(item => !(item.id === id && item.size === size)));
    };
    ...
    const iniciarCobro = (metodoPago) => {
        if (carritoPOS.length === 0) {
            mostrarAlerta("Error: El carrito está vacío", "error");
            return;
        }
        setConfirmacion({
            visible: true,
            mensaje: `¿Confirmar pago de $${mathPOS.total.toLocaleString('es-CL')}?`,
    ...
    const ejecutarCobro = async () => {
        ...
        try {
            const payload = {
                subTotal: mathPOS.subtotal,
                taxAmount: mathPOS.iva,
                totalAmount: mathPOS.total,
                paymentMethod: metodo,
                items: carritoPOS.map(item => ({
                    productId: item.id,
                    quantity: item.cantidad,
                    size: item.size,
                    unitPrice: item.precioFinal, 
                    price: item.precioFinal,     
                    precioEnCompra: item.precioFinal
                }))
            };
                ...
                <AnimatePresence>
                    {alerta.visible && (
                        <motion.div 
                            ...
                            className={`pos-toast ${alerta.tipo}`}
                        >
                            <i className={`fas ${alerta.tipo === 'success' ? 'fa-check-circle' : 'fa-exclamation-triangle'} me-2 fs-5`}></i>
                            {alerta.mensaje}
                        </motion.div>
                    )}
                </AnimatePresence>
            ...
            <AnimatePresence>
                {confirmacion.visible && (
                    <div className="pos-modal-backdrop" onClick={() => setConfirmacion({ visible: false, mensaje: '', metodoPago: '' })}>
                        <motion.div 
                            className="pos-modal-content text-center"
                        ...
                        >
                            ...
                            <div className="d-flex gap-2 mt-4">
                                <button className="flex-fill pos-btn-outline" onClick={() => setConfirmacion({ visible: false, mensaje: '', metodoPago: '' })}>
                                    Cancelar
                                </button>
                                <button className="flex-fill pos-btn-primary" onClick={ejecutarCobro}>
                                    Confirmar
                                </button>
                            </div>
                        </motion.div>
                            ...
                            >
                                ...
                                <div className="d-flex flex-wrap gap-2 mb-2">
                                    {productoSeleccionado.variants.map(v => (
                                        <button 
                                            key={v.size} className="pos-size-btn" disabled={v.stock <= 0}
                                            onClick={() => {
                                                agregarAlPOS(productoSeleccionado, v.size);
                    ...
                    <motion.div className="col-lg-8" variants={cardVariants}>
                        <div className="pos-card h-100 d-flex flex-column pb-3">
                            ...
                            <div className="row g-3 pos-scrollable p-4 flex-grow-1" style={{ maxHeight: '72vh', overflowY: 'auto' }}>
                                {/* LOGICA DE CARGA INCORPORADA AL CATÁLOGO */}
                                {cargando ? (
                                    ...
                                    productosFiltrados.map(prod => {
                                        ...
                                        return (
                                            <div className="col-6 col-md-4 col-xl-3" key={prod.id}>
                                                <motion.div 
                                                    whileTap={{ scale: 0.95 }}
                                                    className={`pos-product-card h-100 ${prod.stock < 1 ? 'opacity-50' : ''}`}
                                                ...
                                                >
                                                    <div className="pos-product-img-wrapper">
                                                        ...
                                                        {isSale && (
                                                            <span className="position-absolute top-0 start-0 m-2 badge bg-danger border border-danger">OFERTA</span>
                                                        )}
                                                    </div>
                    ...
                    <motion.div className="col-lg-4" variants={cardVariants}>
                        <div className="pos-card h-100 d-flex flex-column">
                            ...
                            <div className="card-footer p-4" style={{ backgroundColor: 'rgba(0,0,0,0.3)', borderTop: '1px solid rgba(255,255,255,0.08)'}}>
                                <div className="d-flex justify-content-between small text-secondary mb-2 fw-semibold">
                                    <span>Subtotal (Neto)</span><span className="text-white opacity-75">${mathPOS.subtotal.toLocaleString('es-CL')}</span>
                                </div>
                                <div className="d-flex justify-content-between small text-secondary mb-3 fw-semibold">
                                    <span>IVA (19%)</span><span className="text-white opacity-75">${mathPOS.iva.toLocaleString('es-CL')}</span>
                                </div>
                                <div className="d-flex justify-content-between fw-bolder fs-4 text-white mb-4 pt-3" style={{ borderTop: '2px dashed rgba(255,255,255,0.15)', letterSpacing: '-0.02em' }}>
                                    <span>TOTAL</span><span>${mathPOS.total.toLocaleString('es-CL')}</span>
                                </div>

                                <div className="d-grid gap-2">
                                    <button 
                                        className="pos-btn-success d-flex justify-content-center align-items-center"
                                        disabled={carritoPOS.length === 0 || procesando}
                                        onClick={() => iniciarCobro('Efectivo')}
                                    >
                                        {procesando ? <i className="fas fa-circle-notch fa-spin me-2"></i> : <i className="fas fa-money-bill-wave me-2"></i>}
                                        COBRAR EN EFECTIVO
                                    </button>
                                    <button 
                                        className="pos-btn-primary d-flex justify-content-center align-items-center"
                                        disabled={carritoPOS.length === 0 || procesando}
                                        onClick={() => iniciarCobro('Tarjeta')}
                                    >
                                        {procesando ? <i className="fas fa-circle-notch fa-spin me-2"></i> : <i className="fas fa-credit-card me-2"></i>}
                                        COBRAR CON TARJETA
                                    </button>
                                </div>
                            </div>
